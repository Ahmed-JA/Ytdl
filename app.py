...
def youtube_worker(url, folder_name, mode, quality, sort_by, engine_name):
    global job_stats
    create_cookie_file()
    job_stats.update({"active": True, "log": "🔍 تحليل الرابط...", "total_done": 0, "skipped": 0, "start_time": time.time()})
    
    # تحسين الرابط لدعم القنوات مباشرة
    target_url = url.strip()
    if ('youtube.com/@' in target_url or '/channel/' in target_url or '/c/' in target_url) and '/videos' not in target_url and '/shorts' not in target_url and '/streams' not in target_url:
        target_url = target_url.rstrip('/') + '/videos'

    try:
        # إعدادات الاستخراج لدعم القنوات والقوائم
        ydl_opts = {
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # استخراج الفيديوهات سواء كان الرابط فيديو واحد، قائمة، أو قناة
            if 'entries' in res:
...
# --- بقية الكود (UI, Routes) تبقى كما هي ---
UI = """
<!DOCTYPE html>
