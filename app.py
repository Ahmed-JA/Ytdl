...
# --- قاعدة بيانات المحركات (إضافة AK-A) ---
ENGINES = {
...
def youtube_worker(url, folder_name, mode, quality, sort_by, engine_name):
    ...
    job_status.update({"active": True, "log": f"🔍 تحليل القناة (المحرك: {engine_name})...", "total_done": 0, "total_count": 0})
...
<head>
    <meta charset="UTF-8">
    <title>Koyeb Radar Pro v25</title>
    <link href="https://fonts.googleapis.com/css2?family=Cairo&display=swap" rel="stylesheet">
    <style>
        body { background: #0d1117; color: #c9d1d9; font-family: 'Cairo'; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; }
        .box { width: 90%; max-width: 480px; background: #161b22; padding: 30px; border-radius: 20px; border: 1px solid #30363d; }
        input, select, button { width: 100%; padding: 12px; margin: 8px 0; border-radius: 10px; border: 1px solid #30363d; background: #0d1117; color: #fff; }
        button { background: #238636; border: none; font-weight: bold; cursor: pointer; }
        .bar { height: 10px; background: #21262d; border-radius: 5px; margin-top: 20px; overflow: hidden; }
        .fill { height: 100%; background: #58a6ff; width: 0%; transition: 0.5s; }
    </style>
</head>
<body>
    <div class="box">
        <h2 style="text-align:center;color:#58a6ff;">🛰️ KOYEB RADAR v25</h2>
        <input id="url" placeholder="رابط يوتيوب">
        <input id="folder" placeholder="اسم المجلد">
        ...
        <div id="log" style="text-align:center;margin-top:10px;font-size:14px;">جاهز</div>
    </div>
    <script>
        function start() {
            const data = { 
        ...
        async function poll() {
            ...
            document.getElementById('log').innerText = d.log + " | " + d.current_file;
