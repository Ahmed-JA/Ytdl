    ...
    try:
        ...
        for i, video in enumerate(videos):
            ...
            for sub, fmt, ext in tasks:
                ...
                job_stats.update({"current_file": f"[{sub}] {v_title[:25]}...", "log": f"📡 محرك {engine_name} ينقل {processed}"})
        ...
        job_stats.update({"log": "✅ اكتمل العمل بنجاح", "active": False, "current_file": "تم الانتهاء"})
...
<head>
    ...
    <title>RADAR v37.1 - 'خاص يوتيوب'</title>
    <style>
        body { background: #000; color: #0f0; font-family: 'Courier New', monospace; padding: 15px; }
        .box { max-width: 500px; margin: auto; border: 1px solid #0f0; padding: 25px; border-radius: 20px; background: #050505; box-shadow: 0 0 20px #0f03; }
        input, select, button { width: 100%; padding: 12px; margin: 8px 0; background: #000; color: #0f0; border: 1px solid #0f0; border-radius: 10px; font-size: 14px; box-sizing: border-box; }
        button { background: #0f0; color: #000; font-weight: bold; cursor: pointer; border: none; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
        .bar { background: #111; height: 14px; border: 1px solid #0f0; margin: 15px 0; border-radius: 7px; overflow: hidden; }
        #fill { background: #0f0; height: 100%; width: 0%; transition: 0.5s; }
        .stat { display: flex; justify-content: space-between; font-size: 13px; padding: 6px 0; border-bottom: 1px solid #030; }
        .label { color: #888; }
        #curr { font-size: 12px; color: #0fa; text-align: center; margin-top: 10px; min-height: 15px; }
    </style>
</head>
<body>
    <div class="box">
        <h2 style="text-align:center; margin-bottom:20px;">🛰️ رادار v37.1 (خاص يوتيوب)</h2>
        <input id="u" placeholder="رابط يوتيوب">
        <input id="f" placeholder="اسم المجلد داخل 'خاص يوتيوب'">
        <div class="grid">
            <select id="e"><option value="AK-A">المحرك AK-A</option><option value="AK1">المحرك AK1</option></select>
        ...
        <div id="log" style="text-align:center; font-weight:bold;">الحالة: جاهز</div>
        ...
        <div id="curr">- انتظار التشغيل -</div>
    </div>
    <script>
        function start(){
