...
def run_task(url, folder):
    ...
    token = get_token()
    status.update({"active": True, "log": "📡 جاري البحث عن الفيديو..."})
    
    ydl_opts = {
        'format': 'bestaudio/best',
        'outtmpl': '%(title)s.%(ext)s', # استخدام اسم الفيديو الأصلي
        'cookiefile': c_path,
        'noplaylist': True
    }
    ...
    try:
        ...
        if os.path.exists(filename):
            ...
            # تنظيف اسم الملف من الرموز التي قد ترفضها Dropbox
            clean_name = "".join(x for x in info['title'] if x.isalnum() or x in " -_").strip()
            db_path = f"/خاص يوتيوب/{folder}/{clean_name}.{ext}"
            
            with open(filename, "rb") as f:
                requests.post("https://content.dropboxapi.com/2/files/upload", 
                    headers={"Authorization": f"Bearer {token}", "Dropbox-API-Arg": json.dumps({"path": db_path, "mode": "overwrite"})}, data=f)
            
            os.remove(filename)
            status["log"] = "✅ تم الرفع بنجاح!"
        else:
            status["log"] = "❌ السيرفر لم يجد الملف المحمل"
...
def index():
    ...
    <body style="background:#000;color:#d4af37;text-align:center;padding:20px;font-family:sans-serif;">
        <div style="border:1px solid #d4af37;border-radius:15px;padding:20px;max-width:400px;margin:auto;">
            ...
            <button onclick="start()" id="b" style="width:100%;background:#d4af37;color:#000;padding:15px;font-weight:bold;border-radius:10px;">سحب الآن</button>
        ...
        <script>
            ...
            async function poll(){
                ...
                if(d.active) setTimeout(poll,2000); else document.getElementById("b").disabled=false;
            }
        </script>
    </body>
    ''')
