...
# الكوكيز التي أرسلتها (سيقوم الكود بتحويلها لملف تلقائياً)
...
# دالة لتحويل النص إلى ملف Netscape Cookie File
def create_cookie_file():
    with open("cookies.txt", "w") as f:
        f.write("# Netscape HTTP Cookie File\n")
        f.write("# This file was generated by Koyeb script\n")
        for cookie in RAW_COOKIES.split(';'):
            try:
                if '=' in cookie:
                    key, value = cookie.strip().split('=', 1)
                    # كتابة الكوكيز بالصيغة التي يعشقها yt-dlp
                    f.write(f".youtube.com\tTRUE\t/\tFALSE\t2698716888\t{key}\t{value}\n")
                    f.write(f".google.com\tTRUE\t/\tFALSE\t2698716888\t{key}\t{value}\n")
            except: pass
...
def run_task(url, folder):
    global status
    create_cookie_file() # إنشاء الملف قبل البدء
    token = get_token()
    status.update({"active": True, "log": "🍪 تم حقن الكوكيز، جاري الهجوم..."})
    ...
    ydl_opts = {
        ...
        'quiet': True,
        'cookiefile': 'cookies.txt',  # استخدام الملف الذي أنشأناه
    ...
    try:
        ...
        if os.path.exists("final.mp3"):
            ...
            os.remove("final.mp3")
            status["log"] = "✅ تم! انتصرنا عليه!"
        else:
            status["log"] = "❌ فشل التحميل (تأكد من الرابط)"
    except Exception as e:
        err = str(e)
        if "Sign in" in err:
            status["log"] = "🔒 الكوكيز منتهية الصلاحية، تحتاج تحديث"
        else:
            status["log"] = f"⚠️ خطأ: {err[:30]}"
    status["active"] = False

@app.route('/')
def index():
    return render_template_string('''
    <body style="background:#111;color:#0f0;text-align:center;padding:30px;font-family:monospace;">
        <div style="border:2px dashed #0f0;padding:20px;max-width:400px;margin:auto;">
            <h2>☢️ Nuclear Cookie Radar v11</h2>
            <input id="u" placeholder="Youtube Link" style="width:100%;padding:10px;margin:10px 0;background:#000;color:#fff;border:1px solid #333;">
            <input id="f" placeholder="Folder Name" style="width:100%;padding:10px;margin:10px 0;background:#000;color:#fff;border:1px solid #333;">
            <button onclick="start()" style="width:100%;padding:10px;background:#0f0;color:#000;font-weight:bold;cursor:pointer;">LAUNCH</button>
            <h3 id="l" style="margin-top:20px;">READY</h3>
        </div>
        <script>
            function start(){
